...
const getEvents = catchAsync(async (req, res) => {
  // Allow filtering by group_id if passed in query (e.g., /v1/events?group_id=123)
  const filter = pick(req.query, ['group_id']);
  const events = await eventService.queryEvents(filter);
  res.send(events);
});
